...
export async function runDurableGeneration<T>(
  ...
  try {
    ...
    if (remembered) return await pollJob<T>(workspace, remembered);
    ...
    if (!response.ok || !body.id) {
      forgetJob(workspace);
      if (isLocalBrowser()) return await directGeneration<T>(endpoint, payload);
      throw new Error(body.error || `Unable to queue generation (${response.status}).`);
    }

    return await pollJob<T>(workspace, body.id);
  } finally {
...
export async function resumeDurableGeneration<T>(workspace: string): Promise<T | null> {
  ...
  setGenerationActivity(true);
  try {
    return await pollJob<T>(workspace, id);
  } finally {
    setGenerationActivity(false);
  }
}
