...
export const authSora: CSSProperties = { fontFamily: "'Sora', ui-sans-serif, system-ui, sans-serif" };
export const authManrope: CSSProperties = { fontFamily: "'Manrope', ui-sans-serif, system-ui, sans-serif" };
...
export function AuthShell({ children, title, subtitle }: { children: ReactNode; title: string; subtitle: string }) {
  return (
    ...
    >
      ...
      <Link
        to="/"
        style={{ ...authSora, fontWeight: 800, fontSize: 24, letterSpacing: "-0.02em", color: AC.text, textDecoration: "none", padding: "40px 0 36px" }}
      >
      ...
      >
        <h1 style={{ ...authSora, fontWeight: 800, fontSize: 26, margin: 0, color: AC.text }}>{title}</h1>
        <p style={{ margin: "8px 0 26px", fontSize: 14, lineHeight: 1.6, color: AC.text2 }}>{subtitle}</p>
        {children}
      </div>
    </div>
  );
...
export function AuthDivider() {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "20px 0" }}>
      ...
      <span style={{ fontSize: 12, fontWeight: 700, color: AC.muted, textTransform: "uppercase", letterSpacing: "0.08em" }}>ou</span>
      <span style={{ flex: 1, height: 1, background: AC.border }} />
    </div>
  );
}
...
export function AuthField({
  ...
  return (
    <label style={{ display: "block", marginBottom: 16 }}>
      <span style={{ display: "block", marginBottom: 7, fontSize: 13, fontWeight: 700, color: AC.text2 }}>{label}</span>
      <input
